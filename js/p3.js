...
function showImages() {
   ...
   for (var j =0; j<allBoxes.length; j++) { 
      var id = document.getElementsByName("boxName")[j].getAttribute("id").replace("clonedbox_",'').replace('sample_box_','');                    
...
function manageUserClick(object) {        
    ...
    if (idClickedOnOddTime != idOfObjectClicked) {  
        ...
        // if clicked even time, we need to compare to see if matched
           ...
           // hide it after 1 second if they do not match!
           if (!matched) {
              setTimeout('hideImage("'+idOfObjectClicked+'")',1000);
              setTimeout('hideImage("'+idClickedOnOddTime+'")',1000);
           }
...
function processyMatchedStatus(matched) {
   ...
   if (matched) {
      matchedCount +=1;
      $('#status').html('Total matched images:'+(matchedCount*2) +". Congrats, you have a good memory!") ;      
   } else {
      $('#status').html('Total matched images:'+(matchedCount*2) +". Sorry, try again!") ;      
   }
   ...
   if (matchedCount ==10) {
      ...
      finalMessage += percentage+"% of your clicks are good\n";
      $('#status').html('Congrats,You have found all of the matching images!\n'+ finalMessage);      
    }
...
function startTiming(){
 ...
 $('#imageKeywords').change(function(){
     $("#status").html('');
     var keywordEntered = $.trim($(this).val()); 
     console.log('keywordEntered '+keywordEntered);
     var defaultArray = keywordsUsed.split(',');

     if (isElementInArray(keywordEntered, defaultArray) || isElementInArray(keywordEntered+"s", defaultArray) ) {
         $("#status").html("The word you entered is being used by this program, please type a different word");
         $("#status").css('color','red');
     } else {
        keywordsUsed += ','+keywordEntered;
        getWebImagesByKeyword(keywordEntered);

     }
 });

// logic for populate images, 
...
$("#startGame").click(function (){
   ...
   // give user 10 seconds to remember the game
...
function fadeOutImages(timeToFadeOut){
  ...
  $('[name="boxName"]').fadeIn('fast', function(){startGame(); });
}
